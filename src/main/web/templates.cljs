...
(rum/defc header []
  [:header
   {:class "header"}
   [:h1 {:class "title"} [:a {:href "/"} "EONS :: IO ()"]]
   [:nav
    {:class "nav"}
    [:ul
     {:class "nav__links"}
     [:li {:class "nav__link"} [:a {:href "/about/"} "about"]]]]
   [:button
    {:class "theme-toggle theme-btn -light",
   ...
   [:button
    {:class "theme-toggle theme-btn -dark",
  ...
  [:footer
   ...
   [:section
    ...
    [:a
     ...
     [:svg
      {:viewBox "0 0 24 24", :fill "currentColor", :width "24"}
      [:path
       {:fill-rule "evenodd",
        :clip-rule "evenodd",
        :d
