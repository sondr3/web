...
export class Server {
  ...
  private watch(): void {
    fs.watch(this.site.config.assets.style, (type, name) => {
      ...
      if (type === "change" || type === "rename") {
        ...
        void renderStyles(
          this.site,
          path.join(this.site.config.assets.style, "style.scss"),
        );
  ...
  private buildServer(): http.Server {
    return http.createServer((request, response) => {
      ...
      const contentType = Server.findMimetype(mimetypes, extension) ??
        "application/octet-stream";

      fs.readFile(
        path.join(this.site.config.out, filePath),
        (error, content) => {
          if (error) {
